...
function NumeroALetras(num: number, currency: string): string {
  const data = {
    numero: num,
    enteros: Math.floor(num),
    centavos: Math.round(num * 100) - Math.floor(num) * 100,
    letrasCentavos: '',
    letrasMonedaPlural: currency === 'MXN' ? 'PESOS' : 'DOLARES',
    letrasMonedaSingular: currency === 'MXN' ? 'PESO' : 'DOLAR',

    letrasMonedaCentavoPlural: 'CENTAVOS',
    letrasMonedaCentavoSingular: 'CENTAVO',
  }

  data.letrasCentavos = `${data.centavos.toString().padStart(2, '0')}/100 ${currency}`
